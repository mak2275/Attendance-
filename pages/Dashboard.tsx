...
const Dashboard: React.FC<DashboardProps> = ({ students, existingRecords, onSave }) => {
  ...
  // Load local data for selected date
  useEffect(() => {
  ...
  const reportText = useMemo(() => {
    ...
    reportLines.push(`Total Absent: ${uniqueAbsenteesCount}`);
    ...
    <div className="flex flex-col h-screen overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto bg-slate-50">
        ...
        <div className="p-3 space-y-2">
          ...
          {/* Absent Report Preview Integrated Below the List */}
          <div className="mt-8 mb-10">
            <div className="bg-white border border-slate-200 shadow-xl rounded-[24px] overflow-hidden flex flex-col">
              ...
              <div className="px-5 py-4 bg-slate-50/80 backdrop-blur-sm border-b flex items-center justify-between">
                <div className="flex items-center gap-3">
                   ...
                   <div className="flex flex-col">
                     ...
                     <span className="text-[8px] text-slate-400 font-bold uppercase tracking-widest leading-none">Real-time Summary</span>
                   </div>
                </div>
                <button 
                  onClick={handleCopy} 
              ...
              <div className="p-5 bg-white border-t">
                ...
                >
                  <UserCheck size={20} /> Submit & Cloud Sync
                </button>
                <p className="text-center text-[8px] text-slate-400 font-bold uppercase tracking-widest mt-3">Saves to local storage and remote class session</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white w-full max-w-sm rounded-[32px] p-8 shadow-2xl space-y-6 animate-in zoom-in duration-200">
            ...
            <div className="space-y-4">
              <div className="bg-amber-50 border border-amber-100 p-4 rounded-2xl">
                <p className="text-[10px] text-amber-700 font-bold leading-relaxed uppercase text-center">Enter the same code on all devices to sync classroom attendance automatically.</p>
              </div>
              ...
              {lastSynced && (
                <div className="text-center flex items-center justify-center gap-2 pt-2 text-slate-400">
                  <Clock size={10} />
                  <span className="text-[9px] font-bold uppercase tracking-widest">Last Sync: {new Date(lastSynced).toLocaleTimeString()}</span>
                </div>
              )}
            </div>
          </div>
